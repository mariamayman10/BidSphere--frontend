...
function SignUp() {
  ...
  return (
    <div className="min-h-screen auth-page flex justify-center items-center py-15 sm:px-10 px-5">
      ...
      <div className="flex flex-col items-center h-fit md:w-150 w-full pt-20 pb-10 sm:px-10 px-7 rounded-2xl bg-[#0000001f]">
        ...
        <div className="w-full mt-10">
          <form onSubmit={signup}>
            ...
            <div className="flex items-center gap-5 mb-10">
              ...
              <div className="flex bg-surfacetonal-200 p-1 rounded-3xl w-fit">
                {["USER", "SELLER"].map((item) => (
                  <button
                    className={`px-5 py-1.5 rounded-2xl text-surface-500 ${type === item ? "bg-primary" : ""}`}
                    onClick={(e) => {
                      e.preventDefault();
                      setType(item);
                    }}
            ...
            <Btn className="rounded-3xl w-full text-light">Sign Up</Btn>
          </form>
        </div>
      </div>
    </div>
